...
class SearchConfig(BaseModel):
    ...
    embedding_model: str = Field(
        default="sentence-transformers/all-MiniLM-L6-v2", description="嵌入模型"
    )
...
class ValidationConfig(BaseModel):
    ...
    tools: Dict[str, ValidationToolConfig] = Field(
        default_factory=dict, description="验证工具配置"
    )
...
class ConfigManager:
    ...
    def _apply_env_overrides(self) -> None:
        ...
        env_mappings = {
            ...
            "CURSORRULES_TEMPLATES_DIR": ("templates_dir", str),
        ...
        default_tools = {
            "python": {
                ...
                "pylint": ValidationToolConfig(
                    name="pylint", command="pylint", args=["--disable=C0103,C0114"]
                ),
                "black": ValidationToolConfig(
                    name="black", command="black", args=["--check", "--diff"]
                ),
                "mypy": ValidationToolConfig(
                    name="mypy", command="mypy", args=["--ignore-missing-imports"]
                ),
            },
            "javascript": {
            ...
            "cpp": {
                "cppcheck": ValidationToolConfig(
                    name="cppcheck", command="cppcheck", args=["--enable=all", "--xml"]
                ),
                "clang-tidy": ValidationToolConfig(
                    name="clang-tidy", command="clang-tidy", args=["-checks=*"]
                ),
            },
            "markdown": {
                "markdownlint": ValidationToolConfig(
                    name="markdownlint", command="markdownlint", args=["--json"]
                )
            },
        }
...
def get_config(config_path: str = None):
    ...
    加载配置，优先级：
    1. 显式传入 config_path
    2. 环境变量 CURSORRULES_CONFIG
    3. 默认 configs/cursorrules.yaml
    """
    import os
    import re
    ...
    if not config_path:
        config_path = os.environ.get("CURSORRULES_CONFIG", "configs/cursorrules.yaml")
    if not Path(config_path).exists():
        raise FileNotFoundError(f"未找到配置文件: {config_path}")
    with open(config_path, "r", encoding="utf-8") as f:
        content = f.read()
        # 屏蔽 !!python/object/apply 等 tag
        if "!!python/object" in content or "tag:yaml.org,2002:python/object" in content:
            raise RuntimeError(
                f"配置文件 {config_path} 包含不支持的 YAML tag，请用文本编辑器清理！"
            )
    ...
    def assign(d, obj):
        for k, v in d.items():
            if isinstance(v, dict) and hasattr(obj, k):
                assign(v, getattr(obj, k))
            elif hasattr(obj, k):
                setattr(obj, k, v)

    assign(data, config)
    # 环境变量覆盖
    config.rules_dir = os.environ.get("CURSORRULES_RULES_DIR", config.rules_dir)
    config.templates_dir = os.environ.get(
        "CURSORRULES_TEMPLATES_DIR", config.templates_dir
    )
    config.server.log_level = os.environ.get(
        "CURSORRULES_LOG_LEVEL", config.server.log_level
    )
    return config
