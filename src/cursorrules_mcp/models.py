...
class ContentType(str, Enum):
    ...
    OCEANOGRAPHIC_ANALYSIS = "oceanographic_analysis"
    GEOPHYSICAL_MODEL = "geophysical_model"
    CLIMATE_MODEL = "climate_model"
    SPACE_SCIENCE_DATA = "space_science_data"
...
class RuleApplication(BaseModel):
    ...
    contexts: List[str] = Field(
        default_factory=list, description="Application contexts"
    )
...
class RuleValidation(BaseModel):
    ...
    severity: ValidationSeverity = Field(
        default=ValidationSeverity.WARNING, description="Validation severity"
    )
...
class CursorRule(BaseModel):
    ...
    # Rule definition
    rules: List[RuleCondition] = Field(
        ..., description="Rule conditions and guidelines"
    )
    ...
    conflicts_with: List[str] = Field(
        default_factory=list, description="Conflicting rule IDs"
    )
    overrides: List[str] = Field(
        default_factory=list, description="Overridden rule IDs"
    )
...
class KnowledgeItem(BaseModel):
    ...
    embedding_vector: Optional[List[float]] = Field(
        None, description="Semantic embedding"
    )

    # Metadata
    source: Optional[str] = Field(None, description="Content source")
    quality_score: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Quality rating"
    )
...
class MCPContext(BaseModel):
    ...
    intent_tags: List[str] = Field(
        default_factory=list, description="Intent-derived tags"
    )
    ...
    applied_rules: List[str] = Field(
        default_factory=list, description="Previously applied rule IDs"
    )

    # Preferences
    style_preferences: Dict[str, Any] = Field(
        default_factory=dict, description="User style preferences"
    )
    consistency_level: str = Field(
        default="medium", description="Desired consistency level"
    )
...
class ValidationResult(BaseModel):
    ...
    validation_time: Optional[datetime] = Field(
...
class ApplicableRule(BaseModel):
    ...
    matched_conditions: List[str] = Field(
        default_factory=list, description="Matched conditions"
    )
    application_context: Dict[str, Any] = Field(
        default_factory=dict, description="Application context"
    )
    match_reasons: List[str] = Field(
        default_factory=list, description="Why this rule matched"
    )
...
class EnhancedPrompt(BaseModel):
    ...
    injected_rules: List[str] = Field(
        default_factory=list, description="Injected rule IDs"
    )
    context_summary: str = Field(default="", description="Context summary")
    priority_strategy: str = Field(
        default="weighted", description="Priority application strategy"
    )
