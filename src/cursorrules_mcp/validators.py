...
class PythonFlake8Validator(BaseValidator):
    ...
    def _parse_flake8_line(
        self, line: str, temp_file: str
    ...
    def _parse_pylint_item(self, item: Dict[str, Any]) -> Optional[ValidationIssue]:
        ...
        try:
            ...
            severity = severity_map.get(
                item.get("type", "").lower(), ValidationSeverity.WARNING
            )
...
class JavaScriptESLintValidator(BaseValidator):
    ...
    def _parse_eslint_message(
        ...
        try:
            ...
            severity = severity_map.get(
                message.get("severity", 1), ValidationSeverity.WARNING
            )
    ...
    def _parse_markdownlint_issue(
        ...
        try:
            return ValidationIssue(
                ...
                suggestion=self._get_markdownlint_suggestion(
                    issue_data.get("ruleNames", [""])[0]
                ),
            )
        except Exception as e:
...
class ValidatorFactory:
    ...
    _validators = {
        ...
        "javascript": {"eslint": JavaScriptESLintValidator},
        "markdown": {"markdownlint": MarkdownLintValidator},
    }

    @classmethod
    def create_validator(
        cls, language: str, tool_name: str, config: ValidationToolConfig
...
class ValidationManager:
        ...
        for language, tools in validation_tools.items():
            ...
            for tool_name, tool_config in tools.items():
                validator = ValidatorFactory.create_validator(
                    language, tool_name, tool_config
                )
                if validator:
            ...
            if self.config.validation.parallel_validation:
                ...
                for tool_name, validator in language_validators.items():
                    if validator.enabled:
                        task = self._validate_with_tool(
                            validator, content, file_path, tool_name
                        )
                        tasks.append(task)
    ...
    def get_available_validators(self) -> Dict[str, List[str]]:
        ...
        for language, validators in self.validators.items():
            result[language] = [
                tool_name
                for tool_name, validator in validators.items()
                if validator.enabled
            ]
        return result
    ...
    def is_tool_available(self, language: str, tool_name: str) -> bool:
        """检查验证工具是否可用"""
        return (
            language in self.validators
            and tool_name in self.validators[language]
            and self.validators[language][tool_name].enabled
        )
...
"""
        ...
        for issue in result.issues[:3]:  # 显示前3个问题
            print(
                f"  {issue.line_number}:{issue.column_number} {issue.severity.value} {issue.message}"
            )
